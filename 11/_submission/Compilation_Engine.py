...
class Compliation_Engine:
    ...
    def compile_subroutine_dec(self):
        while self.tokenizer.key_word() in {"constructor", "function", "method"}:
            ...
            self.function_type = self.tokenizer.key_word()
            ...
            self.parse_keyword()
            self.parse()
            self.subroutine_name = f"{self.class_name}.{self.tokenizer.identifier()}"
            self.parse_identifier()
    ...
    def compile_subroutine_body(self):
        ...
        self.vm_writer.write_function(self.subroutine_name, self.symbol_table.var_count(Variable_Kind.VAR))

        if self.function_type == "constructor":
            self.vm_writer.write_push(Segment_Type.CONST, self.symbol_table.var_count(Variable_Kind.FIELD))
            self.vm_writer.write_call("Memory.alloc", 1)
            self.vm_writer.write_pop(Segment_Type.POINTER, 0)
        elif self.function_type == "method":
            self.vm_writer.write_push(Segment_Type.ARG, 0)
            self.vm_writer.write_pop(Segment_Type.POINTER, 0)
    ...
    def compile_let(self):
        ...
        if self.tokenizer.symbol() == "[":
            #  push arr
            self.vm_writer.write_push(self.kind_to_segment(self.symbol_table.kind_of(variable)), self.symbol_table.index_of(variable))
            ...
            # add
            self.vm_writer.write_arithmetic(Command_Type.ADD)
            ...
            # push temp 0
            # pop that 0
            self.vm_writer.write_pop(Segment_Type.TEMP, 0)
            self.vm_writer.write_pop(Segment_Type.POINTER, 1)
            self.vm_writer.write_push(Segment_Type.TEMP, 0)
            self.vm_writer.write_pop(Segment_Type.THAT, 0)
            ...
            self.vm_writer.write_pop(self.kind_to_segment(self.symbol_table.kind_of(variable)), self.symbol_table.index_of(variable))
    ...
    def compile_subroutine_call(self):
        ...
        if self.tokenizer.symbol() == ".":
            if self.symbol_table.kind_of(name) != Variable_Kind.NONE:
                self.vm_writer.write_push(self.kind_to_segment(self.symbol_table.kind_of(subroutine)), self.symbol_table.index_of(subroutine))
                n_args += 1
                subroutine = self.symbol_table.type_of(subroutine)

            subroutine += self.tokenizer.symbol()
            self.parse_symbol()
            subroutine += self.tokenizer.identifier()
            self.parse_identifier()

        else:
            subroutine = f"{self.class_name}.{subroutine}"
        ...
        self.vm_writer.write_call(subroutine, n_args)
    ...
    def compile_term(self):
        ...
        # unaryOp term
            ...
            if self.tokenizer.peek(1) in {"(", "."}:
                ...
                self.vm_writer.write_push(self.kind_to_segment(self.symbol_table.kind_of(identifier)), self.symbol_table.index_of(identifier))
                self.parse_identifier()
                ...
                if self.tokenizer.symbol() == "[":
                    ...
                    self.vm_writer.write_pop(Segment_Type.POINTER, 1)
                    # push that 0
                    self.vm_writer.write_push(Segment_Type.THAT, 0)
                    # 
    ...
    def kind_to_segment(self, kind: Variable_Kind) -> Segment_Type:
        return{
            Variable_Kind.ARG: Segment_Type.ARG,
            Variable_Kind.FIELD: Segment_Type.THIS,
            Variable_Kind.STATIC: Segment_Type.STATIC,
            Variable_Kind.VAR: Segment_Type.LOCAL
        }[kind]
